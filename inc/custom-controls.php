...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    */
	...
	/**
	 * Text Radio Button Custom Control for Laura
    *
    * @author Anthony Hortin <http://maddisondesigns.com>
    * @license http://www.gnu.org/licenses/gpl-2.0.html
    * @link https://github.com/maddisondesigns
	 */
	class Laura_Text_Radio_Button_Custom_Control extends WP_Customize_Control {
		/**
		 * The type of control being rendered
		 */
		public $type = 'text_radio_button';
		 ...
		 */
		public function enqueue() {
			wp_enqueue_style( 'laura-custom-controls-css', get_template_directory_uri() . '/inc/customizer-custom-controls/css/customizer.css', array(), '1.0', 'all' );
		}
			...
			<div class="text_radio_button_control">
				...
				<div class="radio-buttons">
					<?php foreach ( $this->choices as $key => $value ) { ?>
						<label class="radio-button-label">
							<input type="radio" name="<?php echo esc_attr( $this->id ); ?>" value="<?php echo esc_attr( $key ); ?>" <?php $this->link(); ?> <?php checked( esc_attr( $key ), $this->value() ); ?>/>
							<span><?php echo $value; // HTML'i ekrana basmak için esc_html kullanmadık ?></span>
						</label>
					<?php } ?>
				</div>
			</div>
